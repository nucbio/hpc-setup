...
LIB_DIR=$INSTALL_DIR/meson/meson-$MESON_VERSION
SRC_DIR=${LIB_DIR}/src

mkdir -p $SRC_DIR
cd $LIB_DIR
wget https://github.com/mesonbuild/meson/releases/download/$MESON_VERSION/meson-$MESON_VERSION.tar.gz
tar -xzf meson-$MESON_VERSION.tar.gz -C $SRC_DIR --strip-components=1
# install
cd $SRC_DIR
python3 setup.py install --prefix $LIB_DIR --optimize=1

make_lua_module meson $MESON_VERSION "$INSTALL_DIR/meson/meson-$MESON_VERSION/local/bin"
