...
TOOL_NAME="bzip2"
SOURCE_URL="https://gitlab.com/bzip2/bzip2/-/archive/bzip2-$BZIP_VERSION/bzip2-bzip2-$BZIP_VERSION.tar.gz"
TARGET_DIR="$INSTALL_DIR/$TOOL_NAME/bzip2-$BZIP_VERSION"
...
wget -q "$SOURCE_URL" -O "${TOOL_NAME}-${BZIP_VERSION}.tar.gz"
cp "${TOOL_NAME}-${BZIP_VERSION}.tar.gz" "$SOURCE_ARCHIVE/"

# 3. Unpack
tar -xzf "${TOOL_NAME}-${BZIP_VERSION}.tar.gz"
cd bzip2-bzip2-$BZIP_VERSION
...
cp libbz2.so.${BZIP_VERSION} "$TARGET_DIR/lib/"
ln -sf libbz2.so.${BZIP_VERSION} "$TARGET_DIR/lib/libbz2.so.1.0"
